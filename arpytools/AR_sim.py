...
def ar_sim(w, A, C, n, ntr, ndisc=10 ** 3):
    # ARSIM	Simulation of AR process.
    # v=ar_sim(w,A,C,n) simulates n time steps of the AR(p) process
    ...
    if np.any(abs(eigval) > 1):
        raise ValueError("The specified AR model is not stable")
        return
    ...
    for itr in range(ntr):
        for k in range(p + 1, ndisc + n + p + 1):
            for j in range(p):
                x[j - 1, :] = (
                    u[k - j - 1, :, itr] @ AT[(j * m + 1) - 1 : (j + 1) * m, :]
                )
            u[k - 1, :, itr] = np.sum(x, axis=0) + randvec[(k - 1) - p, :, itr]

    # Discard the first ndisc time steps and return process vector
    return u[ndisc + p : ndisc + n + p, :, :]
