...
def arqr(v, p, mcor):
    ...
    for itr in range(1, ntr + 1):
        for j in range(1, p + 1):
            ...
            ] = myarr  # myarr.reshape((myarr.shape[0],1)) # TODO: check if this is correct
...
def arord(R, m, mcor, ne, pmin, pmax):
    ...
    R22 = R[
        int(num_p[imax - 1] + 1) - 1 : int(num_p[imax - 1] + m),
        int(num_p[imax - 1] + 1) - 1 : int(num_p[imax - 1] + m),
    ]
    ...
    for p in np.arange(pmax, pmin - 1, -1):
        ...
        if p < pmax:
            ...
            Rp = R[
                int(num_p[i - 1] + 1) - 1 : int(num_p[i - 1] + m),
                int(num_p[imax - 1] + 1) - 1 : int(num_p[imax - 1] + m),
            ]
...
def ar_interp(v, A, extrasamp, Fs):
    ...
    nan_array[:] = np.nan
    exdat = np.concatenate(
        [v[:, 0, 0], nan_array], axis=0
    )  # add nan's to the end of the original signal to future extrapolated samples

    for es in np.arange(extrasamp):
        currsamp = origsamps + es
        # Location of new sample in the vector
        ...
        exdat[currsamp] = np.sum(
            A * np.flip(exdat[(currsamp - arord) : (currsamp)])
        ) + np.sqrt(np.abs(C)) * np.random.randn(1, 1)
...
def adjph(x):
    ...
    for jj in range(m):
        ...
        )  # norm of new imaginary part
        anorm = np.linalg.norm(
            np.cos(phi) * a + np.sin(phi) * b
        )  # norm of new real part
        if bnorm > anorm:
