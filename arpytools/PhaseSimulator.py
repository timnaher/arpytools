...
class PhaseSimulator:
    ...
    def propagate_phase(self):
        ...
        _, taper = get_w_at(
            self.Prop_taper_k, self.WIN_SIZE
        )  # generate a taper for the propagation. Note: this is not necessarily the same as the AR taper
        self.Prop_taper = (
    ...
    def ar_fit_and_extrapolation(self):
        exdat, w, A, C, th = ar_fit_interpolation(
            lfp=self.lfp, PROP_LENGTH=self.PROP_LENGTH, FS=self.FS, AR_ORD=self.AR_ORD
        )
        self.exdat = exdat
        self.w = w
        self.A = A
        ...
        _, taper = get_w_at(
            self.AR_taper_k, self.WIN_SIZE
        )  # generate another taper for the AR analysis. Note: this is not necessarily the same as the Prop taper
        ...
        if self.nChannels > 1:
            ...
            for jj in range(self.nChannels):
                for i in range(len(self.gtruth)):
                    circ_distances[i, jj] = (
                        np.angle(
                            cmath.exp(1j * np.squeeze(self.gtruth[i, jj]))
                            / cmath.exp(1j * np.squeeze(phase[i, jj]))
                        )
                        * np.pi
                        / 180
                    )
            ...
            for i in range(len(self.gtruth)):
                circ_distances[i] = (
                    np.angle(cmath.exp(1j * self.gtruth[i]) / cmath.exp(1j * phase[i]))
                    * np.pi
                    / 180
                )
    ...
    def plot_phdiffusion(self):
        ...
        ax[0].axvline(
            self.window_stop, color="w", linestyle="--", label="critical time"
        )
        ax[1].axvline(
            self.window_stop, color="w", linestyle="--", label="critical time"
        )
